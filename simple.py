...
class SimpleModel:
    def __init__(self):
        ...
        self.buffer = _DEFAULT_BUFFER

    def eval(self, feature):
        pass

    def learn(self, zeroes, ones):
        """
        ...
        if not zeroes:
            self.threshold = ones[0] - _DEFAULT_BUFFER
            return
        ...
        next_i = bisect_left(ones, self.threshold)
        self.buffer = (ones[next_i] - self.threshold) / 2. if next_i < len(ones) else _DEFAULT_BUFFER
